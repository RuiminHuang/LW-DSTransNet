...
class UpBlock_attention_Without_CCA(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2)
        # self.coatt = CCA(F_g=in_channels, F_x=in_channels)
...
class UpBlock_attention_With_CSCE(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        super().__init__()
        self.up = nn.Upsample(scale_factor=2)
        # self.coatt = CCA(F_g=in_channels, F_x=in_channels)
...
class DSTransNet_Student_Adapt_to_HAFNet(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.x1_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = 16, kernel_size=1)
        self.x2_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 2, out_channels = 32, kernel_size=1)
        self.x3_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 4, out_channels = 64, kernel_size=1)
        self.x4_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 8, out_channels = 128, kernel_size=1)
        ...
        self.skip_x1_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = 16, kernel_size=1)
        self.skip_x2_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 2, out_channels = 32, kernel_size=1)
        self.skip_x3_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 4, out_channels = 64, kernel_size=1)
        self.skip_x4_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 8, out_channels = 128, kernel_size=1)

        # self.d1_conv = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = self.teacher_base_channels * 1, kernel_size=1)
        self.d1_conv_HAFNet = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = 16, kernel_size=1)
        self.d2_conv = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = self.teacher_base_channels * 1, kernel_size=1)
        self.d3_conv = nn.Conv2d(in_channels = self.student_base_channels * 2, out_channels = self.teacher_base_channels * 2, kernel_size=1)
        self.d4_conv = nn.Conv2d(in_channels = self.student_base_channels * 4, out_channels = self.teacher_base_channels * 4, kernel_size=1)
...
class DSTransNet_Student_Adapt_to_SDSNet(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.x2_conv = nn.Conv2d(in_channels = self.student_base_channels * 2, out_channels = self.teacher_base_channels * 2, kernel_size=1)
        self.x3_conv = nn.Conv2d(in_channels = self.student_base_channels * 4, out_channels = self.teacher_base_channels * 4, kernel_size=1)
        self.x4_conv = nn.Conv2d(in_channels = self.student_base_channels * 8, out_channels = self.teacher_base_channels * 4, kernel_size=1)

        self.skip_x1_conv = nn.Conv2d(in_channels = self.student_base_channels * 1, out_channels = self.teacher_base_channels * 1, kernel_size=1)
        self.skip_x2_conv = nn.Conv2d(in_channels = self.student_base_channels * 2, out_channels = self.teacher_base_channels * 2, kernel_size=1)
        self.skip_x3_conv = nn.Conv2d(in_channels = self.student_base_channels * 4, out_channels = self.teacher_base_channels * 4, kernel_size=1)
        self.skip_x4_conv = nn.Conv2d(in_channels = self.student_base_channels * 8, out_channels = self.teacher_base_channels * 4, kernel_size=1)
...
class DSTransNet_Student_v18(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.input_head = nn.Conv2d(self.input_channels, self.base_channels * 1, kernel_size=3,  stride=1, padding=1) # 1 -> 8 [1 8 256 256]
        self.down_encoder1 = nn.Conv2d(self.base_channels * 1, self.base_channels * 2, kernel_size=3,  stride=1, padding=1)  # 8 -> 16 [1 16 128 128 ]
        self.down_encoder2 = nn.Conv2d(self.base_channels * 2, self.base_channels * 4, kernel_size=3,  stride=1, padding=1)  # 16 -> 32 [1 32 64 64 ]
        self.down_encoder3 = nn.Conv2d(self.base_channels * 4, self.base_channels * 8, kernel_size=3,  stride=1, padding=1)  # 32 -> 64 [1 64 32 32]
        self.down_encoder4 = nn.Conv2d(self.base_channels * 8, self.base_channels * 8, kernel_size=3,  stride=1, padding=1)  # 64 -> 64 [1 64 16 16]
        ...
        self.mtc = nn.Module()
        self.mtc.forward = lambda x1,x2,x3,x4: (x1,x2,x3,x4,None)
        ...
        self.up_decoder4 = UpBlock_attention_Without_CCA(self.base_channels * 8,  self.base_channels * 4, nb_Conv=2)
        self.up_decoder3 = UpBlock_attention_Without_CCA(self.base_channels * 4,  self.base_channels * 2, nb_Conv=2)
        self.up_decoder2 = UpBlock_attention_Without_CCA(self.base_channels * 2,  self.base_channels * 1, nb_Conv=2)
        self.up_decoder1 = UpBlock_attention_Without_CCA(self.base_channels * 1,  self.base_channels * 1, nb_Conv=2)
        self.output_head = nn.Conv2d(self.base_channels, self.n_classes, kernel_size=(1, 1), stride=(1, 1))
...
class DSTransNet_Student_v19(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v20(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v21(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v22(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v23(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v24(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v25(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.input_head = new_conv_block(self.input_channels, self.base_channels * 1) # 1 -> 8 [1 8 256 256]
        self.down_encoder1 = new_conv_block(self.base_channels * 1, self.base_channels * 2)  # 8 -> 16 [1 16 128 128 ]
        self.down_encoder2 = new_conv_block(self.base_channels * 2, self.base_channels * 4)  # 16 -> 32 [1 32 64 64 ]
        self.down_encoder3 = new_conv_block(self.base_channels * 4, self.base_channels * 8)  # 32 -> 64 [1 64 32 32]
        self.down_encoder4 = new_conv_block(self.base_channels * 8, self.base_channels * 8)  # 64 -> 64 [1 64 16 16]
...
def conv_relu_bn_1(in_channel, out_channel, dirate=1):
    return nn.Sequential(
        nn.Conv2d(
            in_channels=in_channel,
            out_channels=out_channel,
...
class new_conv_block_1(nn.Module):
    """
    Convolution Block
    """

    def __init__(self, in_ch, out_ch, kernel_size=5, stride=1, wt_levels=3):
        super(new_conv_block_1, self).__init__()
        self.conv_layer = nn.Sequential(
            # conv_relu_bn(in_ch, in_ch, 1),
            # conv_relu_bn(in_ch, out_ch, 1),
            # conv_relu_bn(out_ch, out_ch, 1),
        ...
        self.cdc_layer = nn.Sequential(
            CDC_conv_1(in_ch, out_ch), nn.BatchNorm2d(out_ch), nn.ReLU(inplace=True)
        )
        self.dconv_layer = nn.Sequential(
            # conv_relu_bn(in_ch, in_ch, 2),
            # conv_relu_bn(in_ch, out_ch, 4),
        ...
        self.final_layer = conv_relu_bn_1(out_ch * 3, out_ch, 1)
...
class DSTransNet_Student_v26(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.input_head = new_conv_block_1(self.input_channels, self.base_channels * 1, kernel_size=5, stride=1, wt_levels=7) # 3 -> 32 [1 32 256 256]      #2^7 * 5 = 640 （256） 
        self.down_encoder1 = new_conv_block_1(self.base_channels * 1, self.base_channels * 2, kernel_size=5, stride=1, wt_levels=6) # 32 -> 64 [1 64 128 128]  #2^6 * 5 = 320 （256）
        self.down_encoder2 = new_conv_block_1(self.base_channels * 2, self.base_channels * 4, kernel_size=5, stride=1, wt_levels=5) # 64 -> 128 [1 128 64 64]  #2^5 * 5 = 160 （128）
        self.down_encoder3 = new_conv_block_1(self.base_channels * 4, self.base_channels * 8, kernel_size=5, stride=1, wt_levels=4) # 128 -> 256 [1 256 32 32]  #2^4 * 5 = 80 （64）
        self.down_encoder4 = new_conv_block_1(self.base_channels * 8, self.base_channels * 8, kernel_size=5, stride=1, wt_levels=3) # 256 -> 256 [1 256 16 16]  #2^3 * 5 = 40 （32）
...
class DSTransNet_Student_v27(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v28(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.up_decoder4 = UpBlock_attention_With_CSCE(self.base_channels * 8,  self.base_channels * 4, nb_Conv=2)
        self.up_decoder3 = UpBlock_attention_With_CSCE(self.base_channels * 4,  self.base_channels * 2, nb_Conv=2)
        self.up_decoder2 = UpBlock_attention_With_CSCE(self.base_channels * 2,  self.base_channels * 1, nb_Conv=2)
        self.up_decoder1 = UpBlock_attention_With_CSCE(self.base_channels * 1,  self.base_channels * 1, nb_Conv=2)
        self.output_head = nn.Conv2d(self.base_channels, self.n_classes, kernel_size=(1, 1), stride=(1, 1))
...
class AGPC_FusionModule(nn.Module):
    def __init__(self, planes_high, planes_low, planes_out):
        super(AGPC_FusionModule, self).__init__()
...
class DSTransNet_Student_v29(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.up_decoder4 = AGPC_FusionModule(self.base_channels * 8,  self.base_channels * 8, self.base_channels * 8)
        self.up_decoder3 = AGPC_FusionModule(self.base_channels * 4,  self.base_channels * 4, self.base_channels * 4)
        self.up_decoder2 = AGPC_FusionModule(self.base_channels * 2,  self.base_channels * 2, self.base_channels * 2)
        self.up_decoder1 = AGPC_FusionModule(self.base_channels * 1,  self.base_channels * 1, self.base_channels * 1)
        self.output_head = nn.Conv2d(self.base_channels, self.n_classes, kernel_size=(1, 1), stride=(1, 1))


        self.decoder4_conv = nn.Conv2d(in_channels=self.base_channels * 8, out_channels=self.base_channels * 4, kernel_size=1, stride=1, padding=0, bias=False)
        self.decoder3_conv = nn.Conv2d(in_channels=self.base_channels * 4, out_channels=self.base_channels * 2, kernel_size=1, stride=1, padding=0, bias=False)
        self.decoder2_conv = nn.Conv2d(in_channels=self.base_channels * 2, out_channels=self.base_channels * 1, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        if self.deepsuper:
            ...
            # print(gt_4.shape, gt_3.shape, gt_2.shape, gt_1.shape)

            gt_4 = F.interpolate(gt_4, scale_factor=8, mode='nearest')
            gt_3 = F.interpolate(gt_3, scale_factor=4, mode='nearest')
            gt_2 = F.interpolate(gt_2, scale_factor=2, mode='nearest')
            gt_1 = F.interpolate(gt_1, scale_factor=1, mode='nearest')
...
class DSTransNet_Student_v30(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.up_decoder4 = ACM_FusionModule(self.base_channels * 8,  self.base_channels * 8, self.base_channels * 8)
        self.up_decoder3 = ACM_FusionModule(self.base_channels * 4,  self.base_channels * 4, self.base_channels * 4)
        self.up_decoder2 = ACM_FusionModule(self.base_channels * 2,  self.base_channels * 2, self.base_channels * 2)
        self.up_decoder1 = ACM_FusionModule(self.base_channels * 1,  self.base_channels * 1, self.base_channels * 1)
        self.output_head = nn.Conv2d(self.base_channels, self.n_classes, kernel_size=(1, 1), stride=(1, 1))
...
class DSTransNet_Student_v31(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v32(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
...
class DSTransNet_Student_v33(nn.Module):
    def __init__(self, config, input_channels=1, n_classes=2, img_size=256, vis=False, mode='train', deepsuper=True):
