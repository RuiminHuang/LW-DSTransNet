...
class SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        if self.deepsuper:
            self.gt_conv5 = nn.Sequential(nn.Conv2d(in_channels * 8, n_classes, 1))
            self.gt_conv4 = nn.Sequential(nn.Conv2d(in_channels * 4, n_classes, 1))
            self.gt_conv3 = nn.Sequential(nn.Conv2d(in_channels * 2, n_classes, 1))
            self.gt_conv2 = nn.Sequential(nn.Conv2d(in_channels * 1, n_classes, 1))
    ...
    def forward(self, x):
        ...
        # print('x1 shape after CCT:', x1.shape)
        # print('x2 shape after CCT:', x2.shape)
        # print('x3 shape after CCT:', x3.shape)
        # print('x4 shape after CCT:', x4.shape)
        ...
        if self.deepsuper:
            ...
            if self.mode == 'train':
                ...
                return (gt5, gt4, gt3, gt2, out, d0)
            else:
                # return torch.sigmoid(out)
                return [d5, d4, d3, d2, d1], (gt5, gt4, gt3, gt2, out, d0)
        else:
            # return torch.sigmoid(out)
            return out
